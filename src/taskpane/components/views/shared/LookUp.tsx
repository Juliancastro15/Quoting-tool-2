                ...
                <Card key={`${sku.PartNumber}-${idx}`} sx={{ bgcolor: "#ffffff", border: "1px solid #e5e7eb", borderRadius: "6px", padding: "20px", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.05)", transition: "background-color 0.2s ease, transform 0.2s ease, box-shadow 0.2s ease", "&:hover": { bgcolor: "#f9fafb", transform: "scale(1.03)", boxShadow: "0 6px 12px rgba(0,0,0,0.05)", }, }}>
                    <CardContent sx={{ paddingBottom: "0px !important", padding: "0px !important" }}>
                        ...
                        <Box sx={{ textAlign: "center", mt: 2 }}>
                            <Button variant="contained" sx={{ bgcolor: "#004328", "&:hover": { bgcolor: "#003020" }, color: "#fff" }} onClick={() => { setActiveSku(sku); setIsModalOpen(true); showNotification(`Viewing details for ${selectedModel || sku.routerModel}`, "info"); }}>View More</Button>
                        </Box>
                    </CardContent>
                </Card>
            ))}
                ...
                <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)}>
                    <DialogTitle>
                        <Typography variant="h6">
                            <span style={{ fontWeight: 600 }}>Details of : </span>
                            <span style={{ fontWeight: 700 }}>{selectedModel || activeSku.routerModel}</span>
                        </Typography>
                    </DialogTitle>
                    <DialogContent sx={{ display: "flex", flexDirection: "column", gap: 1, padding: "25px", paddingBottom: "0px" }}>
                        {["PartNumber", "Category", "MSRP / Retail Price", "Short Description", "Warranty", "Country of Origin"].map((h) => {
                            const value = h === "Category" ? activeSku.categoryType : activeSku[h];
                            if (!value) return null;
                            const headerText = h.replace(/\n/g, "");
                            return (<Typography key={h} sx={{ color: "#374151" }}><strong>{headerText}:</strong> {value}</Typography>);
                        })}
                    </DialogContent>
